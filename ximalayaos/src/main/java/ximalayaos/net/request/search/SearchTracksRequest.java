...
import ximalayaos.net.model.searchtracks.SearchTracksBean;
 ...
 * @ClassName: SearchTracksRequest
 * @Author: 刘志保
 * @CreateDate: 2019/9/17 15:54
 * @Description: java类作用描述
 */
public class SearchTracksRequest {
    private static final String TAG = SearchTracksRequest.class.getName();

    public static void httpSearchTracksRequest(Context context,String q, int categoryId, int calcDimension,
    ...
    public static void httpRequest(Map<String, String> paramsMap, final IRequest iRequest){
        ...
        try {
            call = RetrofitManager.getService().getSearchTracksList(paramsMap);
                ...
                public void onResponse(Response<ResponseBody> response) {
                    ...
                    try {
                        if (response != null && response.body() != null) {
                             ...
                             */
                            Gson gson = new Gson();
                            SearchTracksBean bean = gson.fromJson(rawJson,SearchTracksBean.class);

                            if (iRequest != null) {
                                iRequest.callBackRequestBean(bean);
                            }
                            Log.e("PostTest", "search tracks ----> : " + bean.getTracks().get(0).getPlay_url_64());
                        }
                    } catch (IOException e) {
                        e.printStackTrace();
                    }
